...
# Environment script for persistent configuration
...
# Add Homebrew to PATH in environment script
print_info "Configuring Homebrew PATH in ${ENV_SCRIPT}..."
...
if ! grep -qF "# Homebrew PATH" "${ENV_SCRIPT}" 2>/dev/null; then
    cat >> "${ENV_SCRIPT}" << EOF
...
EOF
    print_success "Homebrew PATH added to ${ENV_SCRIPT}"
else
    print_info "Homebrew PATH already configured"
fi
...
echo "PATH configured in: ${ENV_SCRIPT}"
echo ""
